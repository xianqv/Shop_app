...
public class CartNumberView extends LinearLayout implements View.OnClickListener {
    ...
    public CartNumberView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CartNumberView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if(attrs !=null){
            ...
            final TintTypedArray a = TintTypedArray.obtainStyledAttributes(getContext(), attrs,
                    R.styleable.CartNumberView, defStyleAttr, 0);

            //当前输入的值
            int val =  a.getInt(R.styleable.CartNumberView_value,0);
            setValue(val);
            //购买的最大值
            int maxVal = a.getInt(R.styleable.CartNumberView_maxValue,0);
            if(maxVal!=0)
                setMaxValue(maxVal);
            //购买的最小值
            int minVal = a.getInt(R.styleable.CartNumberView_minValue,0);
            setMinValue(minVal);
            //输入框的background
            a.recycle();
        }
    ...
    private void initView(){
        ...
        View view = mInflater.inflate(R.layout.num_add_sub,this,true);
